...
"[project]/node_modules/composio-core/constants.js [app-route] (ecmascript)": (function(__turbopack_context__) {
...
"[project]/node_modules/composio-core/index.js [app-route] (ecmascript)": (function(__turbopack_context__) {

var { g: global, __dirname, m: module, e: exports } = __turbopack_context__;
{
'use strict';
var axios = __turbopack_context__.r("[project]/node_modules/axios/dist/node/axios.cjs [app-route] (ecmascript)");
var uuid = __turbopack_context__.r("[project]/node_modules/uuid/dist/esm-node/index.js [app-route] (ecmascript)");
var z = __turbopack_context__.r("[project]/node_modules/zod/lib/index.js [app-route] (ecmascript)");
var clientAxios = __turbopack_context__.r("[project]/node_modules/@hey-api/client-axios/dist/index.cjs [app-route] (ecmascript)");
var zodToJsonSchema = __turbopack_context__.r("[project]/node_modules/zod-to-json-schema/dist/cjs/index.js [app-route] (ecmascript)");
var crypto = __turbopack_context__.r("[externals]/crypto [external] (crypto, cjs)");
var pathModule = __turbopack_context__.r("[externals]/path [external] (path, cjs)");
var tools = __turbopack_context__.r("[project]/node_modules/@langchain/core/tools.cjs [app-route] (ecmascript)");
var ai = __turbopack_context__.r("[project]/node_modules/ai/dist/index.js [app-route] (ecmascript)");
const COMPOSIO_VERSION = `0.5.39`;
const getUUID = ()=>{
    return uuid.v4();
};
...
const getLogger = ()=>{
    ...
    const noop = ()=>{};
    return {
        error: logLevelValue >= LOG_LEVELS.error ? (...args)=>logger.error(addTimestampToMessage(formatErrorMessage(args))) : noop,
        warn: logLevelValue >= LOG_LEVELS.warn ? (...args)=>logger.warn(addTimestampToMessage(formatErrorMessage(args))) : noop,
        info: logLevelValue >= LOG_LEVELS.info ? (...args)=>logger.info(addTimestampToMessage(formatErrorMessage(args))) : noop,
        debug: logLevelValue >= LOG_LEVELS.debug ? (...args)=>logger.debug(addTimestampToMessage(formatErrorMessage(args))) : noop
    };
};
 ...
 */ class ComposioError extends Error {
    constructor(errCode, message, description, possibleFix, metadata, originalError){
        ...
        if (originalError) {
            try {
                originalErrorString = typeof originalError === "object" ? JSON.parse(JSON.stringify(originalError)) : originalError;
            } catch (_e) {
...
const getAPIErrorDetails = (axiosError)=>{
    ...
    const errorCode = statusCode ? API_TO_SDK_ERROR_CODE[statusCode] : COMPOSIO_SDK_ERROR_CODES.BACKEND.UNKNOWN;
    ...
    let genericMessage = "";
    const hasNotReceivedResponseFromBE = errorCode === COMPOSIO_SDK_ERROR_CODES.BACKEND.UNAUTHORIZED || errorCode === COMPOSIO_SDK_ERROR_CODES.BACKEND.RATE_LIMIT || errorCode === COMPOSIO_SDK_ERROR_CODES.BACKEND.SERVER_UNAVAILABLE || errorCode === COMPOSIO_SDK_ERROR_CODES.BACKEND.SERVER_UNREACHABLE;
    ...
    switch(errorCode){
        case COMPOSIO_SDK_ERROR_CODES.BACKEND.BAD_REQUEST:
            ...
            const formattedErrors = Array.isArray(validationErrors) ? validationErrors.map((err)=>JSON.stringify(err)).join(", ") : JSON.stringify(validationErrors);
            return {
        ...
        default:
            ...
            const possibleFix = predefinedError.possibleFix || defaultErrorDetails.possibleFix || "";
            return {
                message,
                description,
                possibleFix,
...
class CEG {
    ...
    static handleNonResponseAxiosError(error) {
        ...
        throw new ComposioError(COMPOSIO_SDK_ERROR_CODES.BACKEND.SERVER_UNREACHABLE, error.message || "Server is unreachable. Please contact tech@composio.dev with the error details.", "Server is unreachable. Please contact tech@composio.dev with the error details.", "Please contact tech@composio.dev with the error details.", metadata, error);
    }
    static throwAPIError(error) {
        ...
        const errorCode = statusCode ? API_TO_SDK_ERROR_CODE[statusCode] || COMPOSIO_SDK_ERROR_CODES.BACKEND.UNKNOWN : COMPOSIO_SDK_ERROR_CODES.BACKEND.UNKNOWN;
...
const ZGetListActionsParams = z.z.object({
    ...
    filterImportantActions: z.z.boolean().optional().describe("Filter important actions"),
    filterByAvailableApps: z.z.boolean().optional().describe("Filter actions by available apps")
});
            ...
            const { data: res } = await apiClient.actionsV2.executeActionV2({
                ...
                body: {
                    ...
                    sessionInfo: {
                        ...
                        sessionId: ((_c = (_b = parsedData.requestBody) === null || _b === void 0 ? void 0 : _b.sessionInfo) === null || _c === void 0 ? void 0 : _c.sessionId) || ComposioSDKContext.sessionId
                    },
...
class ActiveTriggers {
     ...
     */ async get({ triggerId }) {
        ...
        try {
            const parsedData = ZTriggerItemParam.parse({
                triggerId
            });
...
class Apps {
     ...
     */ async getRequiredParams(appId) {
        ...
        try {
            ZGetRequiredParams.parse({
                appId
            });
            const appData = await this.get({
                appKey: appId
            });
     ...
     */ async getRequiredParamsForAuthScheme({ appId, appName, authScheme }) {
        ...
        try {
            ...
            ZGetRequiredParamsForAuthScheme.parse({
                appId: finalAppId,
                authScheme
            });
...
const ZConnectionStatus = z.z.enum([
    "INITIATED",
    "ACTIVE",
    "FAILED"
]);
...
class PusherUtils {
    static getPusherClient(baseURL, apiKey) {
        if (!PusherUtils.pusherClient) {
            ...
            const PusherClient = __turbopack_context__.r("[project]/node_modules/pusher-js/dist/node/pusher.js [app-route] (ecmascript)");
...
const ZTriggerQuery = z.z.object({
    ...
    triggerInstanceIds: z.z.array(z.z.string()).optional().describe("Trigger Instance IDs"),
    ...
    connectedAccountIds: z.z.array(z.z.string()).optional().describe("Connected Account UUIDs"),
    integrationIds: z.z.array(z.z.string()).optional().describe("Integration IDs"),
    showEnabledOnly: z.z.boolean().optional().describe("Show Enabled triggers only")
});
    ...
    async subscribe(fn, filters = {}) {
        ...
        const shouldSendTrigger = (data)=>{
            if (Object.keys(filters).length === 0) return true;
                ...
                return (!filters.appName || data.appName.toLowerCase() === filters.appName.toLowerCase()) && (!filters.triggerId || data.metadata.id.toLowerCase() === filters.triggerId.toLowerCase()) && (!filters.connectionId || data.metadata.connectionId.toLowerCase() === filters.connectionId.toLowerCase()) && (!filters.triggerName || data.metadata.triggerName.toLowerCase() === filters.triggerName.toLowerCase()) && (!filters.entityId || data.metadata.connection.clientUniqueUserId.toLowerCase() === filters.entityId.toLowerCase()) && (!filters.integrationId || data.metadata.connection.integrationId.toLowerCase() === filters.integrationId.toLowerCase());
            }
        };
...
class Entity {
        ...
        try {
            ...
            if (!latestAccount) {
                for (const connectedAccount of connectedAccounts.items){
                    if ((finalApp === null || finalApp === void 0 ? void 0 : finalApp.toLocaleLowerCase()) === connectedAccount.appName.toLocaleLowerCase()) {
     ...
     */ async disableTrigger(triggerId) {
        ...
        try {
            await this.activeTriggers.disable({
                triggerId: triggerId
            });
            return {
                status: "success"
...
const setAxiosClientConfig = (axiosClientInstance)=>{
    ...
    axiosClientInstance.interceptors.response.use((response)=>{
        ...
        const requestStartTime = (_b = response.config.metadata) === null || _b === void 0 ? void 0 : _b.startTime;
        const responseTime = requestStartTime ? Date.now() - requestStartTime : 0;
        const status = response.status;
        const requestId = response.headers["x-request-id"] || ((_c = response.config.metadata) === null || _c === void 0 ? void 0 : _c.requestId);
...
class Composio {
     ...
     */ constructor(config = {}){
        ...
        ComposioSDKContext.sessionId = (config === null || config === void 0 ? void 0 : config.allowTracing) !== false ? getUUID() : undefined;
    ...
    async getExpectedParamsForUser(params) {
        ...
        const hasRelevantTestConnectors = params.authScheme ? ((_k = (_j = appInfo.testConnectors) === null || _j === void 0 ? void 0 : _j.filter((connector)=>connector.authScheme === params.authScheme)) === null || _k === void 0 ? void 0 : _k.length) > 0 : ((_l = appInfo.testConnectors) === null || _l === void 0 ? void 0 : _l.length) > 0;
        ...
        if (!schema) {
            throw new Error(`No supported auth scheme found for \`${String(app)}\`, ` + "Please create an integration and use the ID to " + "get the expected parameters.");
        }
        integration = await this.integrations.create({
            appId: appInfo.appId,
            name: `integration_${timestamp}`,
...
const readFileContent = async (path)=>{
    try {
        const content = __turbopack_context__.r("[externals]/fs [external] (fs, cjs)").readFileSync(path);
        return {
...
const FILE_DOWNLOADABLE_PROCESSOR = async ({ actionName, toolResponse })=>{
    ...
    for (const [key, value] of Object.entries(toolResponse.data)){
        ...
        if (!(fileData === null || fileData === void 0 ? void 0 : fileData.s3url)) continue;
...
class ComposioToolSet {
     ...
     */ constructor({ apiKey, baseUrl, runtime, entityId, connectedAccountIds, allowTracing } = {}){
        ...
        const clientApiKey = apiKey || getEnvVariable("COMPOSIO_API_KEY") || getUserDataJson().api_key;
    ...
    async getToolsSchema(filters, _entityId, _integrationId) {
        ...
        const toolsWithCustomActions = customActions.filter((action)=>{
            ...
            return (!filters.actions || filters.actions.some((name)=>name.toLowerCase() === (actionName === null || actionName === void 0 ? void 0 : actionName.toLowerCase()))) && (!filters.tags || filters.tags.some((tag)=>tag.toLowerCase() === "custom"));
        });
        const toolsActions = [
    ...
    isCustomAction(action) {
        return this.userActionRegistry.getActions({
            actions: [
                action
            ]
    ...
    async executeAction(functionParams) {
        ...
        if (await this.isCustomAction(action)) {
            if (!accountId) {
                const toolName = await this.userActionRegistry.getToolName({
                    action
                });
        ...
        if (connectedAccountKeys.length > 0) {
            const actionDetails = await this.actions.get({
                actionName: action
            });
...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const { APPS, ACTIONS } = __turbopack_context__.r("[project]/node_modules/composio-core/constants.js [app-route] (ecmascript)");
...
//# sourceMappingURL=node_modules_composio-core_dec3fec1._.js.map